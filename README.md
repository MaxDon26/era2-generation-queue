...
- **Персистентность (localStorage, тз.md:4.7):**
  - при восстановлении выбран вариант **«продолжить»**: `running`-задачи остаются `running`
    с сохранённым `progress` (движок продолжает с того же места, а не заново). `startedAt`
    пересчитывается как `now − прошедшее_время`, чтобы длительность/ETA были консистентны
    (`resumeRunning`).
  - persist-обёртка хранит версию схемы; при несовпадении версии — сброс на сид. `phase`
    и `undo` не персистятся.
